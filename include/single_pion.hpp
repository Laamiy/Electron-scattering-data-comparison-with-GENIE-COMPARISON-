...
#include "Common/GCEventLoopDriver.h"
#include "Common/GCNameConv.h"
#include "Common/GCPlex.h"
#include "Common/GCStyle.h"
#include "Interfaces/GExDataI.h"
...
enum class target : int { PROTON = 1000010010, NEUTRON = 1000000010 };
...
struct Kinematics 
{
  ...
  Kinematics(double Q2, double W, double eps, double theta, double sigma,double unc, double sigmat, double sigmal)
      : q2(Q2), w(W), epsilon(eps), theta_pi(theta), sigma_0(sigma), unc_0(unc),
        sigma_t(sigmat), sigma_l(sigmal)
  {}
  ...
  friend std::ostream &operator<<(std::ostream &stream,const Kinematics &kine)
  {
    char buffer[392];
    sprintf(buffer,
            "|| %14.7e GeV | %14.7e GeV | %14.7e n/a | %14.7e deg | %14.7e GeV "
            "| %14.7e n/a ||",
            kine.q2, kine.w, kine.epsilon, kine.theta_pi, kine.sigma_0,
            kine.unc_0);
    stream << buffer << '\n';
    return stream;
  }
...
public:
  double q2, w, epsilon, theta_pi, sigma_0, sigma_t, sigma_l, unc_0, unc_t,
      unc_l;
}; // Kinematics
...
{
  public:
    Spp_dataset_description(int tgtpdg, const std::string &expt, double E,
                            double w, double q2, double eps, double theta,
                            double sigma_0, double unc, double sigma_t,
                            double sigma_l)
        : m_target_pdg(tgtpdg), m_experiment(expt),
          m_kine(q2, w, eps, theta, sigma_0, unc, sigma_t, sigma_l)
          {}
